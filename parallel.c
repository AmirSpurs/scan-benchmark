...
int main()
{
    ...
    // FIX : we initialize data here. imaganie we have data here
    ...
    scan_seq(data->input, output_seq, size, 0);
    clock_gettime(CLOCK_MONOTONIC, &end_seq);
    elapsed = (end_seq.tv_sec - start_seq.tv_sec) * 1e6 + (end_seq.tv_nsec - start_seq.tv_nsec) / 1e3;
    printf("%f\n", elapsed);
    bool test = true;
    for (int i = 0; i < size; i++)
    {
