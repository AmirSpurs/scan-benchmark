...
for s in $sizes; do
    read -r parallel_time sequential_time roofline_parallel_time roofline_sequential_time < <(./main.out "$s")
    echo "$s,$parallel_time,$sequential_time,$roofline_parallel_time,$roofline_sequential_time" | tee -a "$outfile"
done
